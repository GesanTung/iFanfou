                ...
                if let lparameters = parameter{
                    for item in lparameters.keys{
                        if lparameters[item] is UIImage {
                            let data = UIImageJPEGRepresentation((lparameters[item]as? UIImage)!,0.6)!
                            //let data = UIImagePNGRepresentation((lparameters[item]as? UIImage)!)!
                            multipartFormData.appendBodyPart(data: data, name: String(item), fileName: String(item),mimeType:"application/octet-stream")
                        }else{
