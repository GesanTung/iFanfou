...
class UIHomeViewController: BaseViewController,BaseTableViewDelegate,MGSwipeTableCellDelegate {
    ...
    override func initContentView() {
        ...
        gViewContent.gTableAutoHeight = true
         gViewContent.gTableHasFooter = true
        ...
        if (direction == MGSwipeDirection.RightToLeft){
            ...
            if isSelf==true{
                ...
                (sender: MGSwipeTableCell!) -> Bool in
                   let url = KURLAppDelete  + "?id=" + (data["id"] as! String)
                    self.HttpPost(url, parameter:nil)
                   return true
                })
            }else{
                zhuan =  MGSwipeButton(title: "收藏", backgroundColor: UIColor.redColor(), padding:15,callback: {
                    (sender: MGSwipeTableCell!) -> Bool in
                    let url = KURLAppFavorites  + "?id=" + (data["id"] as! String)
                    self.HttpPost(url, parameter:nil)
                    return true
                })
            }
            let replay =  MGSwipeButton(title: "回复", backgroundColor: UIColor(red: 156/255.0, green: 211/255, blue: 224/255, alpha: 1.0) , padding:15,callback: {
                ...
                controller.repostStatusId = data["id"] as! String
                controller.repostStatusString = (" 转@" + (data.valueForKeyPath("user.name") as? String)! + " " + (data.valueForKeyPath("text") as? String)!)
                self.pushViewController(controller,animated: true)
                return true
            })
            let delete =  MGSwipeButton(title: "转发", backgroundColor: UIColor.lightGrayColor(), padding:15,callback: {
                (sender: MGSwipeTableCell!) -> Bool in
                self.HttpUpload(KURLAppUpLoad, parameter: ["status":("转@" + (data.valueForKeyPath("user.name") as? String)! + " " + (data.valueForKeyPath("text") as? String)!),"repost_status_id":data["id"] as! String])
                return true
            })
